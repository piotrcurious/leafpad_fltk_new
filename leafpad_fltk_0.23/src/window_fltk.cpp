...
int MainWindow::handle(int event) {
    ...
    if (event == FL_SHORTCUT && Fl::event_key() == FL_Escape) {
        if (search_widget->visible()) {
            ...
            return 1; // Event handled, don't close the app
        }
        // If search isn't open, we could return 0 to let the app close,
        // or return 1 to disable Escape-to-close entirely.
        return 1; 
    }
        ...
        if (Fl::event_ctrl()) {
            switch (Fl::event_key()) {
                case 'f':
 				...
 				case 'g':
    				if (search_widget->visible()) {
        			if (Fl::event_state(FL_SHIFT)) { // Check if Shift is held
                ...
                case 'j':
                    {
                        // Note: fl_input is a modal dialog and handles its own Escape key
                        ...
                        if (line_num_str) {
                            ...
                            if (line_num > 0 && line_num <= total_lines + 1) {
                                ...
                                int display_line = editor->get_display_line_from_pos(pos);
                                editor->scroll(display_line, 0);
                            }
                        }
                    }
                    return 1;
...
void MainWindow::resize(int x, int y, int w, int h) {
    ...
    int search_h = 60; // Fixed height for search bar
    if (search_widget->visible()) {
        editor->resize(0, 30, w, h - 30 - search_h);
        search_widget->resize(0, h - search_h, w, search_h);
    } else {
        editor->resize(0, 30, w, h - 30);
        search_widget->resize(0, h, w, search_h); // Move it off-screen/bottom
    }
}
