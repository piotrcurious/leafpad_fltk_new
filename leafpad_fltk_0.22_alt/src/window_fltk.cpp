...
#include <cstring>  // Added for std::string
...
void MainWindow::update_linenumber_width() {
    ...
    int digits = (num_lines > 0) ? (int)log10(num_lines) + 1 : 1;
    if (digits > 6) digits = 6;

    std::string measure_str(digits, '9'); // Create string of '9's to measure width
    fl_font(editor->textfont(), editor->textsize());
    int width = (int)fl_width(measure_str.c_str());
